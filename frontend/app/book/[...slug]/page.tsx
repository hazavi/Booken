...
export default function BookPage() {
  ...
  const bookPath = Array.isArray(params.slug) ? params.slug.join("/") : "";
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        setLoading(true);
        setError(null);
        const response = await getBookDetails(bookPath);
              ...
              <div className="book-page-reviews-list">
                {book_info.media_reviews.slice(0, 3).map((review, index) => (
                  <div key={index} className="book-page-review-item">
                    <p className="book-page-review-content">{review.content}</p>
                    <p className="book-page-review-source">— {review.source}</p>
                  </div>
                ))}
              </div>
            </div>
